...
const api = {
  ...
  users: {
    list: () => ipcRenderer.invoke(IPC.users.list) as Promise<ApiResponse<UserListItem[]>>,
    create: (input: CreateUserInput) => ipcRenderer.invoke(IPC.users.create, input) as Promise<ApiResponse<UserListItem>>,
    setActive: (input: SetUserActiveInput) => ipcRenderer.invoke(IPC.users.setActive, input) as Promise<ApiResponse<UserListItem>>,
    resetPassword: (input: ResetUserPasswordInput) => ipcRenderer.invoke(IPC.users.resetPassword, input) as Promise<ApiResponse<UserListItem>>,
  },
}
